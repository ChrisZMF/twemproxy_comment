...
req_get(struct conn *conn)
{
    ...
    msg = msg_get(conn, true, conn->redis); //创建msg结构，以及msg相关的处理回调
    if (msg == NULL) {
        conn->err = errno;
    }
    return msg;
...
req_put(struct msg *msg)
{
    ...
    msg_tmo_delete(msg);    //msg的rbtree作用？
...
req_done(const struct conn *conn, struct msg *msg)
{
    ...
    if (msg->nfrag_done < msg->nfrag) { //多key命令的子命令回复尚未完成，nfrag是分割的命令数，nfrag_done已回复的命令数
        return false;
    }
    ...
    ASSERT(msg->frag_owner->nfrag == nfragment);    //多key命令的所有子命令都已经回复

    msg->post_coalesce(msg->frag_owner);    //redis_post_coalesce
...
req_server_enqueue_imsgq(struct context *ctx, struct conn *conn, struct msg *msg)
{
    ...
    TAILQ_INSERT_TAIL(&conn->imsg_q, msg, s_tqe);   //消息插入到s_conn的请求输入队列，由事件处理流程req_send_next处理
...
req_client_enqueue_omsgq(struct context *ctx, struct conn *conn, struct msg *msg)
{
    ...
    TAILQ_INSERT_TAIL(&conn->omsg_q, msg, c_tqe);   //将msg插入conn->omsg_q尾部，c_tqe是msg在clientQ中的队列指针
}
...
req_client_dequeue_omsgq(struct context *ctx, struct conn *conn, struct msg *msg)
{
    ...
    TAILQ_REMOVE(&conn->omsg_q, msg, c_tqe);    //从队列conn->omsg_q中剔除msg
}
...
struct msg *
req_recv_next(struct context *ctx, struct conn *conn, bool alloc)   //msg_recv->req_recv_next(ctx, conn, true)
{
    struct msg *msg;
    ...
    msg = req_get(conn);    //创建msg结构
    if (msg != NULL) {
        conn->rmsg = msg;   //msg结构赋值给c_conn的rmsg
    }
...
static rstatus_t
req_make_reply(struct context *ctx, struct conn *conn, struct msg *req) //由proxy构建的rsp
{
    struct msg *rsp;
    ...
    req->done = 1;
    conn->enqueue_outq(ctx, conn, req); //req消息放入c_conn的omsgq
...
req_filter(struct conn *conn, struct msg *msg)
{
     ...
     */
    if (!conn_authenticated(conn)) {    //连接未认证成功的，则消息不推送，由proxy拦截
        msg->noforward = 1;
    }
...
req_forward(struct context *ctx, struct conn *c_conn, struct msg *msg)
{
    ...
    if (!msg->noreply) {    //如果req是需要回复的
        c_conn->enqueue_outq(ctx, c_conn, msg); //msg放入c_conn的输出队列
    }
    ...
    s_conn = server_pool_conn(ctx, c_conn->owner, key, keylen); //选择一个server，即serverconn
    if (s_conn == NULL) {
        /*
    ...
    if (TAILQ_EMPTY(&s_conn->imsg_q)) { //serverconn的imsg_q为空
        status = event_add_out(ctx->evb, s_conn);   //对serverconn注册写事件，准备发送请求内容
        if (status != NC_OK) {
            req_forward_error(ctx, c_conn, msg);
            s_conn->err = errno;
            return;
    ...
    s_conn->enqueue_inq(ctx, s_conn, msg);  //serverconn的输入队列msg入队操作
...
req_recv_done(struct context *ctx, struct conn *conn, struct msg *msg,
              struct msg *nmsg) //msg是已解析部分，nmsg是尚未解析的请求部分
{
    rstatus_t status;
    ...
    conn->rmsg = nmsg;  //将还未处理的部分放入rmsg，后续msg_recv_chain -> req_recv_next 继续处理

    if (req_filter(conn, msg)) {
        return;
    }

    if (msg->noforward) {   //不需要推送给server的消息，proxy拦截的请求则由proxy直接回复
        ...
        status = msg->reply(msg);   //redis_reply 回调处理，构造rsp消息
        if (status != NC_OK) {
            conn->err = errno;
            return;
        }
    ...
    status = msg->fragment(msg, array_n(&pool->server), &frag_msgq);    //多key复杂命令拆分
    if (status != NC_OK) {
        if (!msg->noreply) {
    ...
    if (TAILQ_EMPTY(&frag_msgq)) {  //命令没有被拆分成多个命令
        req_forward(ctx, conn, msg);
        return;
    }

    status = req_make_reply(ctx, conn, msg);    //make reply作用？？
    if (status != NC_OK) {
        if (!msg->noreply) {
            conn->enqueue_outq(ctx, conn, msg);
        }
    ...
    for (sub_msg = TAILQ_FIRST(&frag_msgq); sub_msg != NULL; sub_msg = tmsg) {  //针对拆分命令，依次推送子命令到对应的server
...
struct msg *
req_send_next(struct context *ctx, struct conn *conn)   //conn是s_conn
{
    rstatus_t status;
    struct msg *msg, *nmsg; /* current and next message */

    ASSERT(!conn->client && !conn->proxy);  //请求发送接口，只有server conn可以使用

    if (conn->connecting) {
        server_connected(ctx, conn);    //server_connected 处理？？
    }

    nmsg = TAILQ_FIRST(&conn->imsg_q);  //从请求输入队列中获取消息，在req_forward中 req_server_enqueue_imsgq 插入s_conn
    if (nmsg == NULL) {
    ...
    msg = conn->smsg;   //conn->smsg是conn当前准备发送的msg，初次smsg为NULL；不为空表示？？
    if (msg != NULL) {
        ASSERT(msg->request && !msg->done);
        nmsg = TAILQ_NEXT(msg, s_tqe);
    }

    conn->smsg = nmsg;  //注册到当前准备发送的消息，在 msg_send_chain 中重新置NULL
...
void
req_send_done(struct context *ctx, struct conn *conn, struct msg *msg)  //conn是s_conn
{
    ASSERT(!conn->client && !conn->proxy);
    ...
    conn->dequeue_inq(ctx, conn, msg);  //已发送的消息，从conn->imsg_q队列中移出
     ...
     */
    if (!msg->noreply) {    //noreply在创建msg时就置0，表示需要回复
        conn->enqueue_outq(ctx, conn, msg); //消息放入s_conn->omsg_q队列
    } else {
        req_put(msg);   //作用？？
    }
}
