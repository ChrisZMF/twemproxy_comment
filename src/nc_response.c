...
rsp_recv_next(struct context *ctx, struct conn *conn, bool alloc)
{
    ...
    msg = conn->rmsg;   //设置当前的msg
    if (msg != NULL) {
        ASSERT(!msg->request);
        return msg; //msg不为空则返回msg
    }
    ...
    msg = rsp_get(conn);    //创建msg结构，准备接收response消息
    if (msg != NULL) {
        conn->rmsg = msg;
    }
...
rsp_filter(struct context *ctx, struct conn *conn, struct msg *msg)
{
    ...
    pmsg = TAILQ_FIRST(&conn->omsg_q);  //未处理请求队列，
    if (pmsg == NULL) { //无req待处理回复，则消息无效
    ...
    if (pmsg->swallow) {    //吞没响应，当发生如client断开等情况的时候，需要吞没响应消息
...
rsp_forward(struct context *ctx, struct conn *s_conn, struct msg *msg)
{
    ...
    server_ok(ctx, s_conn); //确定server正常

    /* dequeue peer message (request) from server */
    pmsg = TAILQ_FIRST(&s_conn->omsg_q);    //取peer msg也就是请求消息，同一conn上的请求一定是顺序处理的，依次取出即可
    ASSERT(pmsg != NULL && pmsg->peer == NULL);
    ASSERT(pmsg->request && !pmsg->done);

    s_conn->dequeue_outq(ctx, s_conn, pmsg);    //req出s_conn的输出队列omsgq
    pmsg->done = 1;

    /* establish msg <-> pmsg (response <-> request) link */
    pmsg->peer = msg;   //关联peer关系
    msg->peer = pmsg;

    msg->pre_coalesce(msg); //消息预合并，针对多key命令？？

    c_conn = pmsg->owner;   //找到对应的client连接结构
    ASSERT(c_conn->client && !c_conn->proxy);

    if (req_done(c_conn, TAILQ_FIRST(&c_conn->omsg_q))) {
        status = event_add_out(ctx->evb, c_conn);   //注册输出事件，发送rsp给对应的client
        if (status != NC_OK) {
            c_conn->err = errno;
        }
    }
...
rsp_recv_done(struct context *ctx, struct conn *conn, struct msg *msg,
              struct msg *nmsg) //nmsg是mbuf中尚未解析的消息部分，msg是已经解析了的部分
{
    ASSERT(!conn->client && !conn->proxy);
    ...
    conn->rmsg = nmsg;  //尚未解析的消息

    if (rsp_filter(ctx, conn, msg)) {   //已经解析了的消息，过滤消息
        return;
    }

    rsp_forward(ctx, conn, msg);    //正确的消息准备被推送
}

struct msg *
rsp_send_next(struct context *ctx, struct conn *conn)   //conn是c_conn
{
    rstatus_t status;
    ...
    pmsg = TAILQ_FIRST(&conn->omsg_q);  //c_conn的omsgq，存放已推送给server的req消息，在req_forward后存入
    ...
    if (req_error(conn, pmsg)) {
        ...
        msg = pmsg->peer;   //获取到req的rsp消息内容，返回给调用函数
    }
    ASSERT(!msg->request);

    conn->smsg = msg; //注册到当前准备发送的消息，在 msg_send_chain 中重新置NULL
...
rsp_send_done(struct context *ctx, struct conn *conn, struct msg *msg)
{
    ...
    pmsg = msg->peer;   //msg是rsp，由rsp取得peer消息req
    ...
    conn->dequeue_outq(ctx, conn, pmsg);    //req出c_conn输出队列omsgq

    req_put(pmsg);  //销毁req消息，归还内存等
}
