...
struct mbuf {
    ...
    uint8_t            *pos;    /* read marker */   //读取mbuf游标
    uint8_t            *last;   /* write marker */  //写入mbuf游标
    uint8_t            *start;  /* start of buffer (const) */   //mbuf数据空间起始位置，也就是mbuf空间的起始位置
    uint8_t            *end;    /* end of buffer (const) */ //mbuf数据空间结束位置，保留出header空间后
};

STAILQ_HEAD(mhdr, mbuf);
//也就是:
// struct mhdr {
//     struct mbuf *stqh_first; /* first element */
//     struct mbuf **stqh_last; /* addr of last next element */
// }
