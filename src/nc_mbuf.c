...
_mbuf_get(void)
{
    ...
    buf = nc_alloc(mbuf_chunk_size);    //默认16k=16384
    if (buf == NULL) {
        return NULL;
    }
     ...
     *
     */
    mbuf = (struct mbuf *)(buf + mbuf_offset);  //mbuf_offset为mbuf_chunk_size-sizeof(header)，找到header位置
    mbuf->magic = MBUF_MAGIC;   //header头填入magic魔术字

done:
    STAILQ_NEXT(mbuf, next) = NULL; //next置空
    return mbuf;
}
...
mbuf_get(void)
{
    ...
    buf = (uint8_t *)mbuf - mbuf_offset;    //定位到mbuf起始位置，由于设置header偏移到了header空间位置
    mbuf->start = buf;
    ...
    mbuf->pos = mbuf->start;    //读mbuf游标
    mbuf->last = mbuf->start;   //写mbuf游标
