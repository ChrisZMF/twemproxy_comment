...
{
    rstatus_t status;
    struct server_pool *pool = elem; //一个server_pool到底相当于一个集群还是一个shard？？
    struct conn *p;

    p = conn_get_proxy(pool);   //获取一个conn结构管理监听套接字，owner为server_pool
    if (p == NULL) {
        return NC_ENOMEM;
    }
...
proxy_init(struct context *ctx)
{
    ...
    //ctx->pool即为server_pool，针对每个server_pool的监听port开启监听
