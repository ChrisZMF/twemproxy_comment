...
static struct conn *
_conn_get(void) //获取conn结构的底层函数
{
    struct conn *conn;

    if (!TAILQ_EMPTY(&free_connq)) {    //优先从free队列中拿，节省内存提高效率
        ...
        conn = nc_alloc(sizeof(*conn)); //free队列没有则新申请
        if (conn == NULL) {
            return NULL;
        }
    }
...
struct conn *
conn_get(void *owner, bool client, bool redis)  //创建server、clients的conn结构及回调函数
{
    struct conn *conn;
    ...
    if (conn->client) {
        ...
        conn->enqueue_outq = req_client_enqueue_omsgq; //req_forward后req消息放入c_conn的omsgq
        conn->dequeue_outq = req_client_dequeue_omsgq; //rsp_send_done后req消息出c_conn的omsgq
        ...
        conn->enqueue_inq = req_server_enqueue_imsgq;   //req消息入s_conn的输入队列
        conn->dequeue_inq = req_server_dequeue_imsgq;   //req发送给server完成后，即出队；出输入队列后即入输出队列omsgq
        conn->enqueue_outq = req_server_enqueue_omsgq;  //req发送完成后，出imsgq后，立即入omsgq
        conn->dequeue_outq = req_server_dequeue_omsgq;  //在收到rsp后，req出omsgq
        if (redis) {
          conn->post_connect = redis_post_connect;
          conn->swallow_msg = redis_swallow_msg;    //server回复时可能遇到client已断开，此时不需要再发送消息，故吞没
        } else {
...
struct conn *
conn_get_proxy(struct server_pool *pool)    //创建proxy监听套接字的conn管理结构
{
    struct conn *conn;
...
conn_recv(struct conn *conn, void *buf, size_t size)
{
    ...
    for (;;) {
        n = nc_read(conn->sd, buf, size);   //read函数，读取连接套接字sd，获取接收消息
...
conn_sendv(struct conn *conn, const struct array *sendv, size_t nsend)
{
    ...
    for (;;) {
        n = nc_writev(conn->sd, sendv->elem, sendv->nelem); //实际发送信息到对端
