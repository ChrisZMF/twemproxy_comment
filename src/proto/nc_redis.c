...
redis_parse_req(struct msg *r)
{
    ...
    state = r->state;   //r就是msg，msg创建初始state=0，也就是SW_START
    b = STAILQ_LAST(&r->mhdr, mbuf, next);  //buf空间，msg内容

    ASSERT(r->request);
    ASSERT(state >= SW_START && state < SW_SENTINEL);
    ASSERT(b != NULL);
    ASSERT(b->pos <= b->last);  //mbuf的读游标一定要<=写游标

    /* validate the parsing maker */
    ASSERT(r->pos != NULL);
    ASSERT(r->pos >= b->pos && r->pos <= b->last);
    //r->pos msg的解析游标，b->pos msg内容mbuf的读游标，b->last msg的mbuf的写游标
    ...
    for (p = r->pos; p < b->last; p++) {
        ...
        case SW_REQ_TYPE:
            ...
            case 3:
                if (str3icmp(m, 'g', 'e', 't')) {
                    r->type = MSG_REQ_REDIS_GET;    //标记命令类型，也就是命令是什么
                    break;
                }
    ...
    r->pos = p + 1;
    ASSERT(r->pos <= b->last);
    r->state = SW_START;    //重置parse状态
    r->token = NULL;
    r->result = MSG_PARSE_OK;   //parse结果
...
redis_parse_rsp(struct msg *r)
{
    ...
    if (b->last == b->end && r->token != NULL) {    //mbuf结束，msg内容未结束？
        r->pos = r->token;
        r->token = NULL;
        r->result = MSG_PARSE_REPAIR;
    } else {
...
redis_pre_coalesce(struct msg *r)
{
    ...
    pr->frag_owner->nfrag_done++;   //记录已回复的子命令数
    ...
    case MSG_RSP_REDIS_INTEGER:
        ...
        pr->frag_owner->integer += r->integer;  //del命令类型的回复，则将
        break;
