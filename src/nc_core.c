...
core_ctx_create(struct instance *nci)
{
    ...
    ctx->cf = conf_create(nci->conf_filename);  //创建conf结构，将yaml文件中的内容解析，并保存下来
    if (ctx->cf == NULL) {
        nc_free(ctx);
        return NULL;
    }
    ...
    status = server_pool_init(&ctx->pool, &ctx->cf->pool, ctx); //根据配置信息创建server_pool等
    if (status != NC_OK) {
        conf_destroy(ctx->cf);
        nc_free(ctx);
        return NULL;
     ...
     */
    status = core_calc_connections(ctx);  //计算总的连接数，也就是给client剩下的连接数
    if (status != NC_OK) {
    ...
    ctx->stats = stats_create(nci->stats_port, nci->stats_addr, nci->stats_interval,
                              nci->hostname, &ctx->pool);   //创建stats结构，保存到ctx中，开启监听线程，事件回调stats_loop
    if (ctx->stats == NULL) {
        server_pool_deinit(&ctx->pool);
        conf_destroy(ctx->cf);
        nc_free(ctx);
    ...
    ctx->evb = event_base_create(EVENT_SIZE, &core_core);   //创建eventbase，如epoll，事件数为1000，事件回调为core_core
    if (ctx->evb == NULL) {
    ...
    status = server_pool_preconnect(ctx);   //由于配置有server信息，故可以先连接上server，也可以等有消息到达再连接server
    if (status != NC_OK) {
    ...
    status = proxy_init(ctx);   //初始化proxy，开启所有server_pool的监听
    if (status != NC_OK) {
        server_pool_disconnect(ctx);
        event_base_destroy(ctx->evb);
        stats_destroy(ctx->stats);
...
core_core(void *arg, uint32_t events)
{
    ...
    ctx = conn_to_ctx(conn);    //根据conn获取到ctx对象
