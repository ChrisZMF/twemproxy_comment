...
nc_pre_run(struct instance *nci)
{
    ...
    status = log_init(nci->log_level, nci->log_filename);   //日志文件初始化
    if (status != NC_OK) {
        return status;
    }

    if (daemonize) {
        status = nc_daemonize(1);   //后台运行fork子进程
        if (status != NC_OK) {
            return status;
        }
    }
...
nc_run(struct instance *nci)
{
    ...
    ctx = core_start(nci);  //创建proxy实例nci的ctx
    if (ctx == NULL) {
        return;
    }
    ...
    /* run rabbit run */
    for (;;) {
        status = core_loop(ctx);    //运行epoll核心事件处理
        if (status != NC_OK) {
            break;
        }
    }
...
{
    rstatus_t status;
    struct instance nci;    //proxy实例

    nc_set_default_options(&nci);   //设置默认配置项

    status = nc_get_options(argc, argv, &nci);  //根据命令行输入设置指定配置项
    if (status != NC_OK) {
        nc_show_usage();
        exit(1);
    }
    ...
    if (show_version) {
        ...
        if (show_help) {
            nc_show_usage();    //打印help信息
        }

        if (describe_stats) {   //打印集群、server的stats信息
            stats_describe();
        }
    ...
    if (test_conf) {
        if (!nc_test_conf(&nci)) {  //测试配置文件的语法正确性
            exit(1);
        }
        exit(0);
    }

    status = nc_pre_run(&nci);  //运行前，创建日志、打印、deamon进程等处理
    if (status != NC_OK) {
        nc_post_run(&nci);
        exit(1);
    }
    ...
    nc_post_run(&nci);  //运行后，清除运行信息，如pid文件，准备退出
